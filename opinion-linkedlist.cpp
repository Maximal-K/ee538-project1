...
class AdjGraph {
    ...
    public:
        ...
        void add_edge(int source, int target) {
            Node* new_node = new Node{source, adj[target]};
            adj[target] = new_node;
        }

        Node* neighbors(int node) {
            return adj[node];
        }

};

void build_adj_list(AdjGraph& graph) {
...
int get_majority_friend_opinions(AdjGraph& graph, const int node) {
    // read in all friend opinions
    int count0 = 0;
    int count1 = 0;

    AdjGraph::Node* cur = graph.neighbors(node);
    while (cur != nullptr) {
        int src = cur -> neighbor;
        if (opinions[src] == 1) {
            count1 ++;
        } else {
            count0 ++;
...
bool update_opinions(AdjGraph& graph) {
    bool changed = false;
    vector<int> new_opinions = opinions;

    for (int i = 0; i < total_nodes; i++) {
        if (const int new_opinion = get_majority_friend_opinions(graph, i); opinions[i] != new_opinion) {
            changed = true;
...
int main() {
    ...
    // convert edge list into adjacency class once we know total_nodes
    AdjGraph graph (total_nodes);
    ...
    while ((iteration < max_iterations) && (opinions_changed)) {
        opinions_changed = update_opinions(graph);
        iteration++;
        if (iteration % 1 == 0)
            cout << "Iteration " << iteration << ": fraction of 1's = "
                    << calculate_fraction_of_ones() << endl;
